...
public class EuchreComputerPlayer {
     ...
     * 
     * @param difficulty
     *            this is the difficulty of this euchre computer
     */
    ...
    /**
     * This method will chose which card a computer player should discard after
     * picking up the "up card"
     * 
     * @param whoseTurn
     *            whose turn it is
    ...
    public Card getLeadCard(int whoseTurn) {
        ...
        if (this.difficulty.equals(Constants.EASY)) {
            // Easy, randomly choose the first card.
                ...
                || this.difficulty.equals(Constants.HARD)) {
            ...
            // TODO make hard computer find a card they can lead with that is of
            // another suit that will win.
        }
    ...
    /**
     * When a card has been lead this will determine the card that the computer
     * should play
     * 
     * @param whoseTurn
     *            - whose turn it is
    ...
    /**
     * If the player can beat what has already been played then this will return
     * the card they should play else it will return null
     * 
     * @param whoseTurn
     *            - this is whose turn it is
     * @return card to beat what has been played or null if the player can't
     *         beat the played cards
     */
    private Card canBeatWhatIsPlayed(int whoseTurn) {
        ...
        while (tempTurn != whoseTurn) {
            if (whoseTurn % 2 == tempTurn % 2) {
                ...
                // this gets the card from the position it would be on the
                // gameboard
                        ...
                        for (Card c : playerCards) {
                            if (compareCards(c, otherPlayerCard)) {
                                if (playerWinningCard == null) {
                                    ...
                                    // winning card is greater than current card
                                    // but current card will still win
                                    playerWinningCard = c;
                                }
                            }
                        }
    ...
    /**
     * If the player can beat all cards that are playable then this will return
     * a card that can beat all cards playable, or else it will return null
     * 
     * @param whoseTurn
    ...
    private Card canBeatAllCardsPlayable(int whoseTurn) {
            ...
            if (tempTurn != whoseTurn) {
                if (game.getCardAtPosition(tempTurn + 1) != null) {
                                ...
                                for (Card c : playerCards) {
                                    if (compareCards(c, otherPlayerCard)) {
                                        if (playerWinningCard == null) {
                                            ...
                                            // winning card is greater than
                                            // current card but current card
                                            // will still win
                                            playerWinningCard = c;
                                        }
                                    }
                    ...
                    if (whoseTurn % 2 == tempTurn % 2) {
                        ...
                        // player has not played their card for this trick so we
                        // look at their hand.
        ...
        // check to see if your teammate will be forced to play a better card
        ...
        if (teammateCards != null && playerWinningCard != null) {
            ...
            // This should only happen if all of the teammates playable cards
            // are better than
     ...
     * 
     * @param cards
     *            - input cards for the method to analyze
     * @return an array of scores that should quantify which trump suit would be
     *         best for a player to call
     */
    ...
    /**
     * If the first card is greater than the second card then return true else
     * return false
     * 
     * @param c1
     *            first card
    ...
    private boolean compareCards(Card c1, Card c2) {
        ...
        Card copyCard1 = new Card(c1);
        Card copyCard2 = new Card(c2);
     ...
     * 
     * @param c
     *            the card to find the value for
     * @param tempTrump
     *            the trump suit
     * @return an integer that is representative of the card's score 0 means
     *         that the card given is null
     */
